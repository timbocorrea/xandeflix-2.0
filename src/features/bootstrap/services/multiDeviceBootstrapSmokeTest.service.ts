...
export async function runMultiDeviceBootstrapSmokeTest(): Promise<MultiDeviceBootstrapSmokeResult> {
  ...
  // 6. Test F: Fonte com catálogo legível confirmado via metadata
  const readableConfirmedMetadata = isLocalCatalogReadable({
    sourceId: dummySourceId,
    sourceType: 'm3u',
    status: 'ready',
    startedAt: new Date().toISOString(),
  ...
  const unconfirmedIdleMetadataNotReadable = !isLocalCatalogReadable({
    sourceId: dummySourceId,
    sourceType: 'm3u',
    status: 'idle',
    startedAt: null,
  ...
  const nullMetadataNotReadable = !isLocalCatalogReadable(null);

  const confirmedEmptyCatalogHandled =
    readableConfirmedMetadata &&
    unconfirmedIdleMetadataNotReadable &&
    nullMetadataNotReadable;

  assertCondition(
    confirmedEmptyCatalogHandled,
    'CONFIRMED_EMPTY_CATALOG_HANDLED_FAILED',
  );

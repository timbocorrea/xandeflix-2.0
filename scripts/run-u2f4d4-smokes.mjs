...
const temporaryRoot = path.resolve(
  os.tmpdir(),
  `xandeflix-u2f4d4-smoke-${process.pid}-${Date.now()}`,
);
const smokeEntry = path.resolve(
  repositoryRoot,
  'src/features/localCatalog/services/localCatalogSearchArtworkSmokeTest.service.ts',
);

if (
  !temporaryRoot.startsWith(path.resolve(os.tmpdir()) + path.sep) ||
  !path.basename(temporaryRoot).startsWith('xandeflix-u2f4d4-smoke-')
) {
  throw new Error('U2F4D4_SMOKE_TEMP_PATH_REJECTED');
}
...
try {
  ...
  const result = await smokeModule.runLocalCatalogSearchArtworkSmokeTest();
